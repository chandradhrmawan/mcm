...
if (!isset($_SESSION["user"])) {
 ...
 $id_user = $_GET['id_pelamar'];;
$cek_duplikasi=$koneksi->query("SELECT * FROM biodata_user WHERE id_pelamar = '$id_user'");

$hasil = $cek_duplikasi->fetch_assoc();
...
<div class="container">
     <?php $id_lowongan = $hasil['id_lowongan']; ?>
...
<input type="hidden" name="id_lowongan" value="<?=$id_lowongan?>">
    ...
    <label>Nama :</label>
    <input type="text" name="nama" id="nama" class="form-control" value="<?php echo $hasil['nama']; ?>">
    </div>
    
    
    <div class="form-group">
    <label>Tanggal lahir :</label>
    <input type="date" name="tanggal_lahir" id="tanggal_lahir" class="form-control" value="<?php echo $hasil['tanggal_lahir']; ?>" >   
    </div>
    
    
    <div class="form-group">
    <label>No Telpon :</label>
    <input type="text" maxlength="12" name="no_telp" id="no_telp" class="form-control" value="<?php echo $hasil['no_telp']; ?>" >   
    </div>
    
    
    <div class="form-group">
    <label>Alamat :</label>
    <textarea name="alamat" id="alamat" rows="5" cols="60" class="form-control"  ><?php echo $hasil['alamat']; ?></textarea> 
    </div>

    <div class="form-group">
    <label>Jenis Kelamin :</label>
    <?php if ($hasil['jenis_kelamin']=="Laki-laki") { ?>
    ...
    <input type="radio" name="jenis_kelamin" id="jenis_kelamin" value="Perempuan" checked="">Perempuan    
    <?php } ?>
    </div>
    ...
    <div class="form-group">
    <label>Status :</label>
    <?php if ($hasil['status']=="Lajang") { ?>
    <input type="radio" name="status" id="status" value="Lajang" checked="">Lajang
    <input type="radio" name="status" id="status" value="Menikah">Menikah   
    <?php }else{ ?>
    <input type="radio" name="status" id="status" value="Lajang" >Lajang
    <input type="radio" name="status" id="status" value="Menikah" checked="">Menikah  
    <?php }?>
    </div>
    <!-- kek yang diatas bud -->
    ...
    <div class="form-group">
    <label>No KTP :</label>
    <input type="text" maxlength="16" name="no_ktp" id="no_ktp" class="form-control" value="<?php echo $hasil['no_ktp']; ?>" >    
    </div>
    
    
    <div class="form-group">
    <label>Email :</label>
    <input type="text" name="email" id="email" class="form-control" value="<?php echo $hasil['email']; ?>">   
    </div>
    
    <!-- Ini lewatin aja dulu -->
    		
         
    <div class="form-group">
    <label>Pendidikan :</label>
    <select name="pendidikan" id="pendidikan" class="form-control">
    		<?php if ($hasil['pendidikan'] == "D3") { ?>
    		<option value="D3" selected="">D3</option>
            <option value="S1">S1</option>
            <option value="S2">S2</option>	
    		<?php }elseif ($hasil['pendidikan'] == "S1") { ?>
      ...
      <!--  -->   
          <input type="text" placeholder="Program Studi" name="pendidkan" id="pendidikan" class="form-control" value="<?php echo $hasil['perguruan_tinggi']; ?>"> 
    </div>
    
    <div class="form-group">
    <label> Perguruan Tinggi </label>
    <?php if ($hasil['perguruan_tinggi']=="ptn") { ?>
        <input type="radio" name="perguruan_tinggi" id="perguruan_tinggi" value="ptn" checked="">PTN
    ...
    <?php }else{ ?>
      <input type="radio" name="perguruan_tinggi" id="perguruan_tinggi" value="ptn">PTN
        <input type="radio" name="perguruan_tinggi" id="perguruan_tinggi" value="pts" checked="">PTS
        <?php }?>
    (Terakreditasi)
    </div>
    ...
    <input type="text" name="nama_perguruan_tinggi" id="nama_perguruan_tinggi" class="form-control" value="<?php echo $hasil['nama_perguruan_tinggi']; ?>">   
    </div>
    
    
    <div class="form-group">
    <label> IPK </label>
    <input type="text" maxlength="4" name="ipk" id="ipk" value="0,00" class="form-control" value="<?php echo $hasil['ipk']; ?>">
    </div>
    
    <div class="form-group">
    <label> ID-Skype </label>
    <input type="text" name="id_skype" id="id_skype" value="<?php echo $hasil['id_skype']; ?>">
    </div>

    <div class="form-group">
    <label>Pengalaman Kerja (Tahun) </label>
    <input type="number" name="pengalaman_kerja" id="pengalaman_kerja" class="form-control" value="<?php echo $hasil['pengalaman_kerja']; ?>">       
    </div>
    
    <div class="form-group">
    <label>Deskripsi Singkat</label>
    <textarea name="deskripsi_singkat" id="deskripsi_singkat" rows="5" cols="60" class="form-control"><?php echo $hasil['deskripsi_singkat']; ?></textarea>  
    </div>
